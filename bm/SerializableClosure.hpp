/*
Code borrowed from https://github.com/darabos/pinty
*/

#ifndef _SERIALIZABLE_CLOSURE_HPP_
#define _SERIALIZABLE_CLOSURE_HPP_ 1
...
public:
    ...
    Closure(_Func f) : size_(sizeof(f)), data_(reinterpret_cast<const char*>(new _Func(std::move(f)))) {}

    static Closure Load(const std::string& dump) {
        return Closure((new std::string(dump))->c_str(), dump.size());
    }
...
private:
    Closure(const char* data, size_t size) : data_(data), size_(size) {}
    const char* data_;
    size_t size_;
};
...
public:
    static _Res Call(const Closure& c, _Args... args) {
        return (*reinterpret_cast<const _Func*>(c.data()))(std::forward<_Args>(args)...);
    }
};
...
public:
    ...
    std::string Serialize() const {
        ...
        std::string closure = closure_.Serialize();
        return call + closure;
    }
...
private:
    ...
    Function(_Call call, Closure cl) : call_(call), closure_(cl) {}
